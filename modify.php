...
<html>
	...
	<body bgcolor="grey">
		...
		<form action="modify.php" method="post" align="center">
		...
		<?php
			...
			if (isset($_POST['submit']))
			{
				...
				{
					header("Location: modify.php?modify=error1");
				...
				else
				{
					...
					$query->execute([$user]);
					$result = $query->fetch();
					if ($result['user_name'] != $opass)
					{
						header("Location: modify.php?modify=error2");
						//error("Current Password and/or New Password Incorrect!");
					}
					else
					{
						$query2 = $db->prepare("UPDATE `comments` SET `user_name` = '$user' WHERE `user_name` = ?");
						$query3 = $db->prepare("UPDATE `images` SET `user_name` = '$user' WHERE `user_name` = ?");
						$query4 = $db->prepare("UPDATE `users` SET `user_name` = '$user' WHERE `user_name` = ?");
						$query1 = $db->prepare("UPDATE `likes` SET `user_name` = '$user' WHERE `user_name` = ?");
